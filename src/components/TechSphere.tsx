...
// Complete tech stack with official icons
const techStack = [
  // Frontend
...
const generateSpherePoints = (count: number, radius: number) => {
  ...
  for (let i = 0; i < count; i++) {
    // Map i to [-1, 1] for y coordinate
    const y = 1 - (i / (count - 1)) * 2;
    
    // Calculate radius at current y level
...
const TechIcon = ({ position, tech, groupRef, index }: TechIconProps) => {
  ...
  useFrame((state) => {
    if (iconRef.current && groupRef.current) {
      // Make icon face camera but stay on sphere surface
      const lookAtPosition = new Vector3().copy(camera.position);
      iconRef.current.lookAt(lookAtPosition);
      
      // Hover animation - push icon slightly forward from sphere surface
      if (hovered) {
        const forwardOffset = new Vector3().copy(position).normalize().multiplyScalar(0.4);
      ...
      >
        <div 
          className={`
            ...
            ${hovered 
              ? 'border-cyan-400 shadow-2xl bg-gray-900/95 scale-110' 
              : 'border-cyan-600/60 bg-gray-800/80'
            }
          `}
          style={{
            boxShadow: hovered 
        ...
        >
          <img 
            ...
            style={{ 
              filter: hovered 
                ? 'brightness(1.3) contrast(1.2) drop-shadow(0 0 8px rgba(255,255,255,0.5))' 
                : 'brightness(1.1) contrast(1.1)'
            }}
          />
          {hovered && (
            <div className="absolute -bottom-10 left-1/2 transform -translate-x-1/2 bg-gray-900/95 text-white px-3 py-1 rounded-lg text-sm whitespace-nowrap border border-gray-600 backdrop-blur-sm">
...
const RotatingSphere = () => {
  ...
  const spherePoints = useMemo(() => generateSpherePoints(techStack.length, 3.0), []);
  ...
  return (
    <group ref={groupRef}>
      {/* Wireframe Sphere Only */}
      <Sphere ref={sphereRef} args={[3.0, 32, 32]}>
        <meshBasicMaterial
          color="#4a9eff"
          wireframe
          transparent
      ...
      {/* Tech Icons attached to sphere surface */}
